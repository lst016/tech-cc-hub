...
5. 确认 CodeGraph / pro-workflow 检索链路是否仍存在全量向量算分风险，并加防回归边界。
...
| P1 | CodeGraph / pro-workflow 检索 | 未深修，只在扫描报告中标记 | 确认实际入口并加边界测试 |
...
### 5. CodeGraph / pro-workflow 检索确认

问题：

- 第一轮扫描提到“全量向量算分 + sort”风险，但当前代码中 legacy vector knowledge 已显示 disabled，需要确认实际仍在运行的入口。

方案：

- 梳理当前 active CodeGraph/pro-workflow search path。
...
5. 最后做 CodeGraph/pro-workflow active path tests。
