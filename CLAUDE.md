...
- 能直接回答的问题不要调用工具；只有答案、改动或验证依赖当前外部状态时才用工具。
- 目标文件不明确时，先用一次有范围限制的 **Grep/Glob/rg/find** 收敛到文件和行号，再读取少量命中片段。
- **禁止 ls → cat → grep → cat 碎片链路**。一次搜索或一次只读批量调用能得出结论的，不拆成多次工具调用。
- 默认不要读取超过 200 行；写后验证只看改动附近或决定性输出，不为确认小改动全量重读。
- 只读操作可以批量并发；写入/删除/安装/提交等副作用操作不混入批量调用。
- 证据足够后停止探索；只有出现新错误、新歧义或用户明确要求深入时，才继续追加工具调用。
