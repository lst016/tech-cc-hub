...
export const FIGMA_COMPONENT_DEVELOPMENT_WORKFLOW_HINTS: readonly string[] = [
  ...
  "Figma SVG asset rule: when implementing icons, vectors, logos, or any SVG from Figma, do not redraw or substitute with local icon libraries. Identify the exact icon/vector node and call figma_get_image_urls with format=\"svg\"; preserve the exported path geometry when adapting it to the project.",
  ...
  "Figma truncation safety rule: NEVER implement, generate code, or make UI decisions from truncated Figma data. If figma_read_design returns truncated=true, you MUST stop and use figma_get_node with a single nodeId from progressiveDisclosure.nodeIndex instead. figma_get_node is guaranteed complete for single nodes; figma_read_design is only for browsing structure.",
  "Figma get-node-first rule: for implementation work, prefer figma_get_node over figma_read_design. figma_get_node takes a single nodeId + small depth and returns guaranteed-complete node JSON with no truncation risk. Only use figma_read_design when you need to browse the file structure and discover node IDs.",
  "Figma progressive disclosure enforcement: when figma_read_design results contain truncated=true and progressiveDisclosure, follow the recommendedNextTool and recommendedNextInput immediately. Do not proceed with implementation. Do not attempt to work from jsonPreview (it has been removed for safety). The only path forward is to drill into a smaller node with figma_get_node or figma_summarize_design.",
];
