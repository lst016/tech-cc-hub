...
export const FIGMA_COMPONENT_DEVELOPMENT_WORKFLOW_STEPS: readonly FigmaDevelopmentWorkflowStep[] = [
  ...
  },
  {
    ...
    doneWhen: "The child has nodeId, imagePath, direct multimodal visual evidence or inspect qualityGate.confidence >= 0.75, DOM selector/region, and measurable visual constraints.",
  },
  {
...
export const FIGMA_COMPONENT_DEVELOPMENT_WORKFLOW_HINTS: readonly string[] = [
  ...
  "Figma no-guess rule: do not infer missing layout/spacing/icon details from prose or previous context. For a text-only main model, a node is editable only after an image-backed reference is locked (`figma_export_node_images` + `design_inspect_image`) or a DOM-to-Figma tuple exists. A multimodal main model may inspect the exported reference directly, so design_inspect_image is optional; the reference tuple and measurable verification still apply.",
  ...
  "Figma child component loop: for each child, run figma_export_node_images -> direct multimodal inspection or design_inspect_image for a text-only model -> lock constraints -> implement/reuse local component -> launch/refresh preview -> browser_inspect_styles/browser_query_nodes for DOM evidence -> design_compare_element_to_reference with the locked selector -> patch until <=0.10 or relock.",
