    ...
    toolGroups: [
      {
        ...
        tools: [
          { name: "knowledge_search", description: "First-line retrieval across Agent Cards, RepoWiki chunks, and Memory entries." },
          ...
          { name: "knowledge_index", description: "Maintenance-only generation/refresh for .tech docs and internal vector/FTS indexes." },
          { name: "memory_update", description: "Add, update, or soft-delete structured Memory entries." },
        ],
      },
    ],
    promptHints: [
      "Knowledge retrieval rule: when <knowledge_overview> is enabled and lists relevant Agent Cards or RepoWiki entries, call `mcp__tech-cc-hub-knowledge__knowledge_search` first, then `mcp__tech-cc-hub-knowledge__knowledge_read` for the chosen document before generic `Read`/`Grep`/`Task` source exploration.",
      "Do not use `mcp__tech-cc-hub-knowledge__knowledge_index` to answer a question or to test retrieval. Use it only when the user explicitly asks to generate, refresh, reindex, or update the knowledge base.",
