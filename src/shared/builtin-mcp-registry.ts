    ...
    toolGroups: [
      {
        ...
        tools: [
          ...
          { name: "codegraph_search", description: "Search code symbols and routes before broad source exploration; auto-indexes/syncs first." },
          { name: "codegraph_context", description: "Build compact graph context for a task or requirement; auto-indexes/syncs first." },
          { name: "codegraph_impact", description: "Trace impact radius for a CodeGraph node id; auto-indexes/syncs first." },
          { name: "memory_update", description: "Add, update, or soft-delete structured Memory entries." },
        ],
      },
    ],
    promptHints: [
      "CodeGraph retrieval rule: for each new user turn that needs source-code evidence, treat CodeGraph as the primary code map and call `mcp__tech-cc-hub-knowledge__codegraph_search` or `mcp__tech-cc-hub-knowledge__codegraph_context` before broad `Read`/`Grep`/`Glob`/`Task` source exploration. Fall back to broad source tools only when CodeGraph has no useful result or has identified exact files for focused reading/editing.",
      "CodeGraph freshness rule: CodeGraph tools auto-initialize `.tech/codegraph` when missing and run incremental sync before retrieval.",
