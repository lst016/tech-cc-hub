  ...
  return (
    <div className="grid gap-4">
      <div className="rounded-[28px] border border-ink-900/10 bg-white/86 p-5 shadow-[0_18px_44px_rgba(24,32,46,0.06)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-muted">
              这里放通用参数（例如 skills、hooks、外部工具清单）。当前仅做配置持久化，不参与模型路由策略。
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-4 rounded-2xl border border-ink-900/10 bg-surface px-4 py-3">
        ...
        <p className="mt-1 text-xs text-muted">
          行号、Tab 缩进都开启了，粘贴 JSON 后可直接点击“格式化 JSON”。
        </p>
        <CodeEditor
          id="global-json"
          value={configText}
        ...
        <div className={`mt-3 rounded-xl px-3 py-2 text-xs ${parseError ? "border border-error/20 bg-error-light text-error" : "border border-ink-900/10 bg-surface text-muted"}`}>
