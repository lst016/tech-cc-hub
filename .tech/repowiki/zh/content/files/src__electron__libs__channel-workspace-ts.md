...
> 模块：`electron` · 语言：`typescript` · 行数：162
...
- `ensureChannelWorkspace@0 - 确保渠道工作区存在，创建目录结构和README`
- `recordChannelInboundMessage@0 - 记录入站消息到messages.jsonl日志文件`
- `buildChannelReplyTarget@0 - 构建回复目标对象，包含provider、conversationId和workspaceRoot`
- `sanitizePathSegment@0 - 清理路径片段，防止路径注入和特殊字符问题`
...
export function buildChannelSessionTitle(message: ChannelInboundMessage, workspace: ChannelWorkspace): string {
  ...
  const source = message.channelName?.trim() || message.senderName?.trim() || workspace.conve
... (truncated)
```
