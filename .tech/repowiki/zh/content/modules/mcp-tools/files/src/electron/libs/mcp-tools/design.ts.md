...
- `DesignToolHost@0 - Host 接口，依赖 captureVisible 和 getState 能力`
- `setDesignToolHost@0 - 注入设计工具的 Host 实例`
- `getDesignArtifactDir@0 - 获取设计产物目录路径（userData/design-parity）`
- `createArtifactPath@0 - 生成带时间戳和标签的产物文件路径`
- `captureCurrentView@0 - 捕获 BrowserView 可见区域截图`
- `captureCurrentRegion@0 - 捕获指定区域的截图`
- `summarizeComparisonReport@0 - 生成 JSON 格式的 diff 比对报告，包含差异率、热点区域、统计摘要`
