...
test("chat selection comments stay alive across selectionchange while the popover is focused", () => {
  const source = readFileSync("src/ui/components/EventCard.tsx", "utf8");

  assert.match(source, /document\.addEventListener\("selectionchange", scheduleSelectionCapture\)/);
  assert.match(source, /const popoverHasFocus = Boolean\(activeElement && selectionPopoverRef\.current\?\.contains\(activeElement\)\)/);
  assert.match(source, /if \(!anchorInside && !focusInside\) \{\s+if \(popoverHasFocus\) return;/);
  assert.match(source, /if \(!selection \|\| selectedText\.length === 0 \|\| selection\.rangeCount === 0 \|\| selection\.isCollapsed\) \{\s+if \(popoverHasFocus\) return;/);
