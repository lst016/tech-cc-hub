...
test("prompt composer exposes workflow as a one-shot send mode beside goal mode", () => {
  assert.match(promptComposerSource, /Workflow/);
  ...
  assert.match(promptComposerSource, /onClick=\{onToggleWorkflow\}/);
  assert.match(promptComposerSource, /aria-label=\{workflowEnabled \? "取消本次使用 Workflow" : "本次使用 Workflow"\}/);
  assert.match(promptComposerSource, /title="本次使用 Workflow"/);
  assert.doesNotMatch(promptInputSource, /workflowMode === "auto" \? "off" : "auto"/);
  assert.doesNotMatch(promptComposerSource, /title="SDK 自动 Workflow"/);

  const workflowButtonIndex = promptComposerSource.indexOf("本次使用 Workflow");
  const goalButtonIndex = promptComposerSource.indexOf("开启追求目标模式");
